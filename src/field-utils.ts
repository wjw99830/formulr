import { CompositionEventHandler, useCallback, useEffect, useMemo, useRef } from 'react';
import {
  CallbackNode,
...
export function useMulti<T, R>(func1: (t: T) => R, func2: (t: T) => R, deps?: any[]): (t: T) => void;
 ...
 * ```
 */
export function usePipe<T0, T1, T2>(fn0: (t0: T0) => T1, fn1: (t1: T1) => T2, deps?: any[]): (t0: T0) => T2;
